...
(function() {
  var Errors, FSWatcher, Readable, Stats, Writable, escape, fs, isAppendable, isFunction, isReadable, isWritable, toDate, _path;
  ...
  fs = (function() {
    ...
    fs.prototype.writeSync = function(fd, buffer, offset, length, position) {
      ...
      if (position !== 0) {
        oldData = this.readFileSync(path, {
          encoding: 'utf8'
        });
    ...
    fs.prototype.watch = function(filename, options, listener) {
      ...
      this.statSync(filename).on('modified', function(stats) {
        return watcher.emit('change', 'change', stats._path);
      });
      this.statSync(filename).on('modifiedAttributes', function(stats, event) {
    ...
    fs.prototype.createWriteStream = function(path, options) {
      ...
      ws._write = function(chunk, enc, next) {
        ...
        return _this.write(fd, chunk, 0, chunk.length, position, function() {
